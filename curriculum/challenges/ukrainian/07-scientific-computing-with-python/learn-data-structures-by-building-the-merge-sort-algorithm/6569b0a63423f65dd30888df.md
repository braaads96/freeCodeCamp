...
Видаліть ключове слово `pass`. For the `while` loop's code block, assign `left_part[left_array_index]` to `array[sorted_index]`.

# --hints--

You should have `array[sorted_index] = left_part[left_array_index]` in the body of the loop. Не забудьте видалити ключове слово `pass` з тіла циклу.
